            ...
            <div class="widget-content">
                <div class="row mb-3">
                    <div class="col-sm-12 col-md-3">
                        <label>Buscar (Referencia / Remitente)</label>
                    ...
                    <table class="table table-bordered table-striped mt-1">
                        <thead class="text-white" style="background: #3B3F5C">
                            <tr>
                                ...
                                <th class="table-th text-white">Remitente</th>
                        ...
                        <tbody>
                            @forelse($records as $record)
                                <tr>
                                    ...
                                    <td>
                                        <!-- Try to show associated invoices/customers from relationships -->
                                        @foreach($record->payments as $payment)
                                            @if($payment->sale)
                                                <small class="d-block text-muted">
                                                    Fact: #{{ $payment->sale->invoice_number ?? $payment->sale->id }} 
                                                    - {{ $payment->sale->customer->name ?? 'Consumidor' }} (Abono)
                                                </small>
                                            @endif
                                        @endforeach
                                        @foreach($record->salePaymentDetails as $paymentDetail)
                                            @if($paymentDetail->sale)
                                                <small class="d-block text-muted" style="color: #28a745 !important;">
                                                    Fact: #{{ $paymentDetail->sale->invoice_number ?? $paymentDetail->sale->id }}
                                                    - {{ $paymentDetail->sale->customer->name ?? 'Consumidor' }} (Contado)
                                                </small>
                                            @endif
                                        @endforeach
                                    </td>
                                    <td class="text-center">
                                        @can('zelle_view_details')
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center">No hay registros Zelle coincidentes</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Detalle de Pago Zelle</h5>
                ...
                <div class="modal-body">
                    @if($selectedRecord)
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <h6>Datos del Zelle</h6>
                                <table class="table table-sm table-bordered">
                                    <tr>
                                        <th>Fecha</th>
                                        <td>{{ $selectedRecord->zelle_date->format('d/m/Y') }}</td>
                                    </tr>
                                    <tr>
                                        <th>Referencia</th>
                                    ...
                                    <tr>
                                        <th>Remitente</th>
                                        <td>{{ $selectedRecord->sender_name }}</td>
                                    </tr>
                                    <tr>
                                        <th>Monto</th>
