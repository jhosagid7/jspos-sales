  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final prefs = await SharedPreferences.getInstance();
        await prefs.setInt('user_id', data['customer']['id'] ?? 0);
      ...
      body: Container(
        ...
        child: Column(
          children: [
            ...
            const Icon(Icons.shopping_cart_checkout, size: 100, color: Colors.amberAccent),
            const Text('JSPOS VIP', style: TextStyle(color: Colors.white, fontSize: 36, fontWeight: FontWeight.bold)),
            const Text('Portal Exclusivo para Clientes', style: TextStyle(color: Colors.white70, fontSize: 16)),
            const Spacer(),
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text('Servidor: $_baseUrl  •  Versión VIP: $_appVersion', style: const TextStyle(fontSize: 10, color: Colors.grey)),
                ],
              ),
            ),
          ],
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  _loadUser() async { 
    ...
    setState(() { 
      _appVersion = pkg.version;
      _userName = prefs.getString('user_name') ?? "VIP Customer"; 
  ...
  Future<void> _refreshConfig() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await prefs.setString('deadline', customer['order_deadline_at'] ?? '');
        await prefs.setBool('deadline_active', false);
        _loadUser();
      }
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            ...
            Text(' VIP', style: TextStyle(color: const Color(0xFF00B4D8).withOpacity(0.8), fontWeight: FontWeight.w900, fontSize: 20)),
          ],
        ),
        actions: [
        ...
        child: Column(
          children: [
            UserAccountsDrawerHeader(
              ...
              accountEmail: const Text("Cliente VIP"),
              currentAccountPicture: const CircleAvatar(backgroundColor: Colors.white, child: Icon(Icons.star_rounded, color: Color(0xFF00B4D8), size: 40)),
            ),
              ...
              child: Column(
                children: [
                   Row(
                     mainAxisAlignment: MainAxisAlignment.center,
                     children: [
                       const Text('Copyright 2026 © ', style: TextStyle(color: Colors.grey, fontSize: 11)),
                       const Text('jhonnypirela.dev', style: TextStyle(color: Color(0xFF00B4D8), fontSize: 11, fontWeight: FontWeight.bold)),
                       Text('  v$_appVersion', style: const TextStyle(color: Colors.grey, fontSize: 11)),
                     ],
                   ),
                   ...
                   const Row(
                     ...
                     children: [
                       ...
                       SizedBox(width: 4),
                       Text('0414-7497092', style: TextStyle(color: Colors.grey, fontSize: 11)),
                       SizedBox(width: 15),
                       Icon(Icons.email_outlined, size: 12, color: Colors.grey),
                       SizedBox(width: 4),
                       Text('jhosagid77@gmail.com', style: TextStyle(color: Colors.grey, fontSize: 11)),
                     ],
                   ),
                ],
              ),
        ...
        builder: (ctx) => Container(
          ...
          child: Row(
            ...
            children: [
              ...
              _navIcon(Icons.shopping_bag_outlined, 'Catálogo', false, () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => CatalogScreen(baseUrl: _baseUrl)));
              }),
...
class CatalogScreen extends StatefulWidget {
  ...
  const CatalogScreen({super.key, required this.baseUrl, this.initialCart, this.initialCustomer, this.initialNotes, this.originalOrderId});
...
class _CatalogScreenState extends State<CatalogScreen> {
    ...
    try {
      ...
      setState(() {
        _selectedCustomer = Customer(
          ...
          name: prefs.getString('user_name') ?? 'Mi Cuenta',
        );
      });
  ...
  Widget _customerSelectionHeader() {
    return const SizedBox.shrink(); // Ocultado para perfil VIP
  }
...
class OrdersScreen extends StatefulWidget {
  final String baseUrl;
  const OrdersScreen({super.key, required this.baseUrl});
  @override
...
class _OrdersScreenState extends State<OrdersScreen> {
  ...
  Future<void> _fetchOrders() async {
    ...
    final token = prefs.getString('token');
    try {
      final response = await http.get(Uri.parse('$_baseUrl/api/vip/orders'), headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'}).timeout(const Duration(seconds: 15));
      if (response.statusCode == 200) {
  ...
  Future<void> _sendToOffice(int id) async {
    ...
    final token = prefs.getString('token');
    try {
      final response = await http.post(Uri.parse('$_baseUrl/api/vip/orders/$id/send'), headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
      if (response.statusCode == 200) {
  ...
  Future<void> _deleteOrder(int id) async {
    ...
    final token = prefs.getString('token');
    try {
      final response = await http.delete(Uri.parse('$_baseUrl/api/vip/orders/$id'), headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
      if (response.statusCode == 200) {
  ...
  void _showOrderLogs(int id) async {
    ...
    final token = prefs.getString('token');
    try {
      final response = await http.get(Uri.parse('$_baseUrl/api/vip/orders/$id/logs'), headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
      if (response.statusCode == 200) {
              ...
              itemBuilder: (c, i) {
                ...
                return Container(
                  ...
                  child: ListTile(
                    ...
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (ctx) => PendingSalesScreen(baseUrl: _baseUrl, customer: cust))),
                  ),
                );
              },
            ),
...
class PendingSalesScreen extends StatefulWidget {
  ...
  const PendingSalesScreen({super.key, required this.baseUrl, required this.customer});
  @override
...
class _PendingSalesScreenState extends State<PendingSalesScreen> {
  ...
  _fetchSales() async {
    ...
    final token = prefs.getString('token');
    try {
      final res = await http.get(Uri.parse('$_baseUrl/api/vip/sales/pending?customer_id=${widget.customer.id}'), headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF8F9FA),
      appBar: AppBar(
        title: const Text('Estado de Cuenta', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16)),
        backgroundColor: Colors.white,
        elevation: 0,
        iconTheme: const IconThemeData(color: Color(0xFF00B4D8)),
      ),
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (ctx) => PaymentHistoryScreen(baseUrl: _baseUrl, saleId: s['id'], invoice: s['invoice_number']))),
                      )
                    ],
                  ),
                ),
...
class PaymentHistoryScreen extends StatefulWidget {
  ...
  const PaymentHistoryScreen({super.key, required this.baseUrl, required this.saleId, required this.invoice});
  @override
...
class _PaymentHistoryScreenState extends State<PaymentHistoryScreen> {
          ...
          children: [
            ...
            ClipRRect(
              ...
              child: CachedNetworkImage(
                imageUrl: '${_historyUrl.replaceAll('/api/vip/payments/history', '')}/storage/$imagePath',
  ...
  _fetchHistory() async {
    ...
    _baseUrl = widget.baseUrl;
    final prefs = await SharedPreferences.getInstance();
    _historyUrl = '$_baseUrl/api/vip/payments/history';
    final token = prefs.getString('token');
    try {
...
class UploadPaymentForm extends StatefulWidget {
  ...
  final String invoice;
  final double debt;
  const UploadPaymentForm({super.key, required this.baseUrl, required this.saleId, required this.invoice, required this.debt});
  @override
...
class _UploadPaymentFormState extends State<UploadPaymentForm> {
  ...
  _init() async {
    ...
    final token = prefs.getString('token');
    try {
      final res = await http.get(Uri.parse('$_baseUrl/api/vip/payments/form-data'), headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
  ...
  Future<void> _submit() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('token');
      var request = http.MultipartRequest('POST', Uri.parse('$_baseUrl/api/vip/payments/upload'));
...
class PerformanceDashboardScreen extends StatefulWidget {
  final String baseUrl;
  const PerformanceDashboardScreen({super.key, required this.baseUrl});
  @override
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
               ...
               child: Column(children: [
                 ...
                 _rowInfo("Ventas del Mes", "\$${sales.toStringAsFixed(2)}", isBold: true),
                    ...
                    Expanded(child: InkWell(
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => DebtDetailScreen(baseUrl: _baseUrl))),
            ...
            // Stats Row
            Row(children: [
               Expanded(child: _statCard("Comis. por Cobrar", "\$${commPending.toStringAsFixed(2)}", Icons.account_balance_wallet_rounded, const Color(0xFF2E7D32), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => CommissionDetailScreen(baseUrl: _baseUrl))))),
               const SizedBox(width: 15),
               Expanded(child: _statCard("Comis. Pagadas", "\$${commPaid.toStringAsFixed(2)}", Icons.check_circle_rounded, const Color(0xFF415A77), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => CommissionDetailScreen(baseUrl: _baseUrl))))),
            ]),
...
class CommissionDetailScreen extends StatefulWidget {
  final String baseUrl;
  const CommissionDetailScreen({super.key, required this.baseUrl});
  @override
...
class DebtDetailScreen extends StatefulWidget {
  final String baseUrl;
  const DebtDetailScreen({super.key, required this.baseUrl});
  @override
...
class PaymentAuditScreen extends StatefulWidget {
  final String baseUrl;
  const PaymentAuditScreen({super.key, required this.baseUrl});
  @override
...
class _PaymentAuditScreenState extends State<PaymentAuditScreen> {
  ...
  _load() async {
    ...
    final token = prefs.getString('token');
    try {
      final res = await http.get(Uri.parse('$baseUrl/api/vip/payments/history/global'), headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
