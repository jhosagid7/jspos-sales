    ...
    <div class="card-header">
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <!-- Search Input with AlpineJS List Logic -->
                <div x-data="{
                    selectedIndex: -1,
                    itemCount: 0,
                    ...
                    <tbody>
                        @forelse($cart as $item)
                            ...
                            <tr wire:key="cart-item-{{ $item['id'] }}">
                                <td><span class="badge badge-light text-dark">{{ $item['sku'] }}</span></td>
                                <td>
                                                                    ...
                                                                    <div class="text-muted small">
                                                                        @foreach($currencies as $currency)
                                                                            @if(!$currency->is_primary)
                                                                                @php $convertedPrice = ($item['sale_price'] / $primaryRate) * $currency->exchange_rate; @endphp
                                ...
                                <td>
                                    <div class="input-group input-group-sm" style="width: 120px;">
                                        <div class="input-group-prepend">
                                            <button class="btn btn-dark btn-sm" type="button" onclick="updateQty('{{ $item['id'] }}','decrement')"><i class="fas fa-minus"></i></button>
                                        </div>
                                        <input type="number" wire:blur="updateQty('{{ $item['id'] }}', $event.target.value )"
                                            wire:keydown.enter.prevent="updateQty('{{ $item['id'] }}', $event.target.value )"
                                            class="form-control form-control-sm text-center" value="{{ $item['qty'] }}" id="qty-{{ $item['id'] }}">
                                        <div class="input-group-append">
                                            <button class="btn btn-dark btn-sm" type="button" onclick="updateQty('{{ $item['id'] }}','increment')"><i class="fas fa-plus"></i></button>
                                        </div>
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="6" class="text-center">Agrega productos al carrito</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
