...
trait PdfInvoiceTrait
{
    ...
    private function getInvoiceFooterData(Sale $sale)
    {
        ...
        // Freight
        // Use logic: If broken down, use stored amount? No, user wants config percent.
        // Actually, for specific invoice code, we should reflect the sale conditions.
        // But the user insisted on config value. Let's use config value for consistency with Price List.
        $freightPercent = $sellerConfig ? floatval($sellerConfig->freight_percent) : 0;

        // Commission
        // Use applied commission if available, else config
        $commPercent = $sale->applied_commission_percent ?? ($sellerConfig ? floatval($sellerConfig->commission_percent) : 0);

        // Diff
        $diffPercent = $sale->applied_exchange_diff_percent ?? ($sellerConfig ? floatval($sellerConfig->exchange_diff_percent) : 0);

        // USD Discount
        // Try to get from CreditConfigService or sale
        // For Sales, it might be stored? 'usd_payment_discount' is not on Sale model by default but let's check.
        // If not, use config.
        ...
        // Note: PDF might be generated by someone else than the seller.
